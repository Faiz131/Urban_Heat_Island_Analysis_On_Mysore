...
// Function to calculate LST using Landsat thermal band (ST_B6)
var calculateLST = function(image) {
  ...
  var bt = image.select('ST_B6').multiply(0.00341802).add(149.0).rename('BT'); // Convert ST_B6 to Kelvin
...
Export.image.toDrive({
  image: lstComposite.float(), // Ensure LST values remain in original range (-1 to +1)
  description: 'Mysore_LST_2004_2014_Clipped',
  folder: 'GEE_Exports', // Specify your Google Drive folder
  fileNamePrefix: 'Mysore_LST_2004_2014_Clipped',
  region: mysore,
